...
amnesty = on_command("解封", aliases={"解禁", "解除禁言"}, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER,
                     priority=20)
...
ban_game_switch_on = on_command("开启自由轮盘", aliases={"开启无赌注轮盘"},
                                permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=5)
...
ban_game_switch_off = on_command("关闭自由轮盘", aliases={"关闭无赌注轮盘"},
                                 permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=5)
...
async def S(event: GroupMessageEvent) -> bool:
    switch.setdefault(event.group_id, False)
    return switch[event.group_id]


ban_game = on_command("无赌注轮盘", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | S, aliases={"自由轮盘"},
                      priority=5)
...
async def Ready(event: GroupMessageEvent) -> bool:
