...
from miyuki.database.models import Context, BlackList, Answers, Answer, BanWord
from .config import ChatConfig

plugin_config = ChatConfig.parse_obj(get_driver().config.dict())
...
class MessageData:
    ...
    def _keywords_list(self):
        ...
        return jieba_analyse.extract_tags(
            self.plain_text, topK=plugin_config.KEYWORDS_SIZE)
    ...
    def to_me(self) -> bool:
        """判断是否为艾特机器人"""
        return self.plain_text.startswith(('bot', 'miyuki'))
...
class LearningChat:
    ...
    async def answer(self) -> Optional[Generator[Union[Message, Message], None, None]]:
        ...
        if self.message.is_plain_text and len(self.message.plain_text) <= 1:
            """不回复单个字的对话"""
            return None
        if self.message.plain_text.startswith(('/', '!', '！', '#', '.')):
            return None
    ...
    async def speak() -> Optional[Tuple[int, int, List[Message]]]:
        ...
        for group_id, group_msgs in popularity:
            ...
            if cur_time - latest_time < avg_interval * plugin_config.speak_threshold + basic_delay:
                continue
            ...
            speak_context = await Context.filter(count__gt=plugin_config.answer_threshold,
                                                 time__gt=available_time).all()
            speak_context_right = []
            for context in speak_context:
                for answer in context.answers:
                    if answer.group_id == group_id and answer.time > available_time and answer.count > plugin_config.answer_threshold:
            ...
            messages = [answer.messages
                        ...
                        if answer.count >= plugin_config.answer_threshold
            ...
            speak_list = [speak]
            while random.random() < plugin_config.speak_continuously_probability and len(
                    speak_list) < plugin_config.speak_continuously_max_len:
            ...
            if random.random() < plugin_config.speak_poke_probability:
    ...
    async def _get_context(self):
        ...
        if msgs := LearningChat.message_cache.get(self.message.group_id):
            # 是否在复读中
            if len(msgs) >= plugin_config.repeat_threshold and all(
                    item['raw_message'] == self.message.raw_message for item in msgs[-plugin_config.repeat_threshold + 1:]):
                # 说明当前群里正在复读
        ...
        answer_threshold_choice_list = list(
            range(plugin_config.answer_threshold - len(plugin_config.answer_threshold_weights) + 1, plugin_config.answer_threshold + 1))
        answer_count_threshold = random.choices(answer_threshold_choice_list, weights=plugin_config.answer_threshold_weights)[
            0]
        if self.message.keywords_len == plugin_config.KEYWORDS_SIZE:
            answer_count_threshold -= 1

        cross_group_threshold = 1 if self.message.to_me else plugin_config.cross_group_threshold
        ...
        for answer in context.answers:
            ...
            if self.message.is_image and '[CQ:' not in sample_msg:
                # 图片消息不回复纯文本
                continue
            if not self.message.to_me and sample_msg.startswith(('bot', 'miyuki')):
                continue
        ...
        if 0 < answer_str.count('，') <= 3 and '[CQ:' not in answer_str and random.random() < plugin_config.split_probability:
    ...
    async def _update_message(self):
        ...
        if len(LearningChat.message_cache[self.message.group_id]) > plugin_config.save_count_threshold:
            await LearningChat.persistence(cur_time)
        elif cur_time - LearningChat._late_save_time > plugin_config.save_time_threshold:
            await LearningChat.persistence(cur_time)
        ...
        if context.ban:
            ...
            for ban in context.ban:
                ...
                if ban.group_id == group_id:
                    ...
                    ban_count[ban_key] += 1
                    if ban_count[ban_key] == plugin_config.cross_group_threshold:
                        ban_keywords.answers.append(ban_key)
    ...
    async def clear_up_context():
        ...
        await Context.filter(time__lt=expiration, count__lt=plugin_config.answer_threshold).delete()
