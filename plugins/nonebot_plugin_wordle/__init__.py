...
handles: dict[int, Optional[type[Matcher]]] = {}
...
def stop_game(user_id: str):
    ...
    global handles
    if not any(i.length == game.length for i in games.values()) and handles.get(game.length, None):
        handles[game.length].destroy()
        handles[game.length] = None
...
async def _(
    ...
    global handles
    if not handles.get(length.result, None):
        ...
        handles[length.result] = wordle_word
